...
class DB_D1:
    ...
    def save_data_to_mysql(cls, search_criteria, names, prices, images=None):
        ...
        for name, price, image in zip(names, prices, images):
            if image is None:
                sql = "INSERT INTO products_d1 (search_criteria, name, price, link_img) SELECT %s, %s, %s, NULL FROM DUAL WHERE NOT EXISTS (SELECT * FROM products_d1 WHERE name = %s)"
                cursor.execute(sql, (search_criteria, name.strip(), price.strip(), name.strip()))
            else:
                sql = "INSERT INTO products_d1 (search_criteria, name, price, link_img) SELECT %s, %s, %s, %s FROM DUAL WHERE NOT EXISTS (SELECT * FROM products_d1 WHERE name = %s)"
    ...
    def get_names_to_mysql(cls, search_criteria):
        ...
        query = "SELECT name FROM products_d1 WHERE search_criteria = %s"
    ...
    def get_prices_to_mysql(cls, search_criteria):
        ...
        query = "SELECT price FROM products_d1 WHERE search_criteria = %s"
        cursor.execute(query, (search_criteria,))
    ...
    def get_links_to_mysql(cls,search_criteria):
        ...
        query = "SELECT link_img FROM products_d1 WHERE search_criteria = %s"
        cursor.execute(query, (search_criteria,))
